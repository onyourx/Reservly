# Creating the "Bagsy" Shopify app
...
shopify app config link       # "Create this project as a new app" → yes, name: bagsy
```

`config link` fills `client_id` in `shopify.app.toml`. Then grab the **Client secret**
(Partner dashboard → Apps → bagsy → Overview → Client credentials) and
